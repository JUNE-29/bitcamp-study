...
public class Exam01_3 {
  public static void main(String[] args) {
    ...
    b = 5; //ok
    b = 6; //ok
    // - 리터럴 5, 6은 4바이트 정수 값이다. 5(4바이트) + 6(4바이트)
    // - 정수 리터럴은 기본이 4바이트 크기이지만,
    // - byte 변수에 저장할 수 있다면 허락한다!

    b = 5 + 6; //ok
    // - 리터럴끼리 산술 연산한 결과도 리터럴로 간주한다.
    // - 그 결과 값이 변수의 범위 내의 값이면 허락한다. // byte : -128 ~ 127
    ...
    //   그 변수의 값이 4바이트 보다 작다면(바이트나 쇼트라면),
    //   4바이트로 만들어 연산을 수행한다.
    // - 즉 임시 4바이트 정수 메모리를 만든 다음에  (1byte -> 4byte로 바뀌어지는게 아니다! 임시 4바이트에 복사가 된다는뜻)
