...
//
// # CPU의 실행 시간을 쪼개는 정책 : CPU Scheduling 또는 프로세스 스케줄링
...
// - 문제점:
// ㄴ 우선 순위가 낮은 프로그램인 경우 CPU 시간을 배정 받지 못하는 문제가 발생했다.
// ㄴ 그래서 몇 년이 지나도록 실행되지 않는 경우가 나타났다.
// - 해결책?
// ㄴ CPU 시간을 배정 받지 못할 때 마다 우선 순위를 높여서
// ㄴ언젠가는 실행되게 만들게 했다.
// ㄴ이런 방식을 "에이징(aging) 기법"이라 부른다.
...
// ㄴ 즉 구현(프로그래밍) 하기가 쉽다.
// - 단점:
// ㄴ 프로세스를 그대로 복제하기 때문에 프로세스가 사용하는 메모리도 그대로 복제된다.
// ㄴ 메모리의 낭비가 심하다.
// ㄴ 복제된 프로세스는 독립적이기 때문에 실행 종료할 때도 일일이 종료해야 한다.
//
// 2) 멀티 스레딩
// - 특정 코드만 분리하여 실행한다.
// - 따라서 프로세스가 사용하는 메모리를 공유한다.
// - 장점:
// ㄴ 프로세스의 힙 메모리를 공유하기 때문에 메모리 낭비가 적다.
// ㄴ 모든 스레드는 프로세스의 종속되기 떄문에 프로세스를 종료하면 스레드도 자동 종료 된다.
// - 단점:
// ㄴ 프로세스 복제 방식에 비해 코드 구현이 복잡하다.
//
// ## 컨텍스트 스위칭(context switching)
// - CPU를 실행 시간을 쪼개 이 코드 저 코드를 실행할 때 마다
// 실행 위치 및 정보(context) 저장하고 로딩하는 과정이 필요하다.
