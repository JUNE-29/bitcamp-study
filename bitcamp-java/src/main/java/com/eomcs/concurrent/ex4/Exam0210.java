...
public class Exam0210 {
  public static void main(String[] args) throws Exception {
    ...
    // ㄴ 그래서 Windows에서 실행할 때는 우선 순위에 영향을 적게 받을 것이다.
    // => Unix, Linux 계열 OS는 우선 순위를 고려한다.
    // ㄴ 그래서 이런 OS에서 실행할 때는 우선 순위에 영향을 받을 것이다.
    //
    // 주의!
    // => JAVA의 캐치프레이즈가 "Write One, Run Anywhere" 이다.
    // => 즉 OS에 상관없이 동일하게 동작하게 만드는 것이 자바의 목적이다.
    // => 그런데 우선 순위에 따라 실행률이 달라지고
    // ㄴ OS마다 차이가 난다면 자바의 목적에 부합하는 것이 아니다.
    // => 그래서 가능한 OS에 영향을 덜 받는 방식으로 코딩해야 한다.
    // => 이러한 이유로 스레드를 다룰 때 우선 순위를 고려하는 방시긍로
    // ㄴ 프로그래밍을 하지 말라!
  }
}
