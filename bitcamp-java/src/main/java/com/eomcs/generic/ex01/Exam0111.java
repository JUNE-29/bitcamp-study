...
public class Exam0111 {
  ...
  public static void main(String[] args) {
    ...
    Member  m = (Member)box1.get(); // 값을 꺼낼 때 형변환 해야 한다. 왜냐하면 Object로 되어있기 때문에
    System.out.println(m);

    ObjectBox box2 = new ObjectBox();
    box2.set(new String("Hello"));
    String str = (String)box2.get(); // 값을 꺼낼 때 원래의 타입으로 형변환 해야 한다.
    System.out.println(str);

    ObjectBox box3 = new ObjectBox();
    box3.set(100); // auto-boxing => Integer.valueOf(100) 으로 바뀜
    int i = (int) box3.get(); // 값을 꺼낼 때 primitive data type을 지정하면
                              // 자동으로 auto-unboxing으로 바꾼다. (Object가 아님)
    System.out.println(i);
    
    // 객체의 타입 별로 Box 클래스를 구분해서 쓰지 않으니 코딩이 편하다.
    // 단 값을 꺼낼 때 원래의 타입의 바꾸기 위해 형변환(type casting)해야 하는 불편함이 있다.
    // 
