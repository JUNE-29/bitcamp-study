...
public class ClientApp {
  ...
  private void processCommand(String command) {
    ...
    try {
      if (!command.startsWith("bitcamp://") // "bitcamp://" 로 시작하지 않는다면
      ) {
      ...
      String[] str = //
          url.substring(0, index)// localhost:9999
              .split(":"); // {"localhost","9999"} substring 호출되서 스트링으로 리턴된것을 나눈다.
    ...
    try (Socket socket = new Socket(host, port);
      ...
      while (true) {
        ...
        if (response.equals("!end!")) {
          // 서버에서 end를 보내면 즉시 보내고 받는거 멈춘다.
          break;
        } else if (response.equals("!{}!")) {
          // 서버에서 이것을 보내면 입력 보내라고 커서가 깜빡인다.
          String input = prompt.inputString("");
          out.println(input); // 사용자가 입력한 것을 서버에게 보낸다.
        } else {
