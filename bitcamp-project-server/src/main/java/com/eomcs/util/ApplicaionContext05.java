...
// - 객체를 생성과 소멸을 관리한다.
//
public class ApplicaionContext05 {
  ...
  public ApplicaionContext05(String packageName) throws Exception {
    ...
    // System.out.println("ApplicaionContext:" + path.getAbsolutePath());

    // 해당 경로를 뒤져서 모든 클래스의 이름을 알아낸다.
    findClasses(path, packageName);

    // 객체를 생성할 때 사용할 concrete class 목록을 준비한다. (순수한 concrete class만)
  ...
  private Object createInstance(Class<?> clazz) throws Exception {
    ...
    // 생성자의 파라미터 정보를 알아낸다. 파라미터 값이 있는지 없는지 확인.
  ...
  private Class<?> findParameterClassInfo(Class<?> paramType) throws Exception {
    ...
    for (Class<?> clazz : concreteClasses) {
      ...
      if (paramType.isInterface()) {
        ...
        // 파라미터가 클래스라면
        // 각각의 클래스에 대해 같은 타입이거나 수퍼클래스 인지 검사한다.
        return clazz;
      }
    }
  ...
  private boolean isConcreteClass(Class<?> clazz) {
    if (clazz.isInterface() // 인터페이스인 경우
      ...
      return false; // 이런 클래스들은 객체를 생성할 수 없다.
    }
    return true; // 그 밖에는 일반 클래스(객체를 생성하는)이다.
  }
