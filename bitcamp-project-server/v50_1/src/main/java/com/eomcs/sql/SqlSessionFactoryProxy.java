...
public class SqlSessionFactoryProxy implements SqlSessionFactory {
  ...
  // 스레드 작업이 끝난후 SqlSession 객체를 제거하고 닫는다.
  ...
  public SqlSession openSession() {
    // 기본으로 자동커밋으로 동작하는 SqlSession을 만들어 리턴한다.
  ...
  public SqlSession openSession(boolean autoCommit) {
    ...
    if (sqlSession == null) {
      // 스레드에 보관된 게 없다면 새로 만든다. SqlSessionProxy에 담아서 리턴. 오리지널말고
      // 왜냐하면 close()를 하지 않게 하기 위해
      sqlSession = new SqlSessionProxy(originalFactory.openSession(autoCommit));
      // 나중에 다른 곳에서 사용하도록 스레드에 보관한다. 당연 SqlSessionProxy에 담겨 보관
