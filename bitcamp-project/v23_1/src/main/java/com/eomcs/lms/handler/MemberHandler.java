...
public class MemberHandler {
  ...
  public MemberHandler(Prompt prompt, List<Member> list) {
    ...
    // Handler가 사용할 List객체(의존 객체; dependency)를 생성자에서 직접 만들지 않고
  ...
  public void updateMember() {
    ...
    newMember.setPassword(prompt.inputString(
        String.format("이메일(%s)? ", oldMember.getPassword()), 
    ...
    newMember.setTel(prompt.inputString(
        String.format("사진(%s)? ", oldMember.getTel()), 
        oldMember.getTel()));
